...
See https://danielwilshirejones.com/rc4-ctr for the full spiel.
...
Options:
  ...
  --server=<url>        URL of server to attack. Required.
...
About
  ...
  I've called RC4-CTR. It poorly borrows ideas from AESs counter mode
  to implement a block cipher from the RC4 key stream. And works like
  this:
  ...
  Python pseudo-code to summarise:
  ...
  This code expects to have access to a mock HTTP API that works as an
  encryption oracle. You can give it the parameters of the above
  function via a GET call to:
